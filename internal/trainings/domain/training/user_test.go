...
func TestIsUserAllowedToSeeTraining(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			Name: "another_attendee_training",
			CreateTraining: func(t *testing.T) *training.Training {
				tr, err := training.NewTraining(
					uuid.New().String(),
					attendee1.UUID(),
		...
		t.Run(c.Name, func(t *testing.T) {
			...
			if c.ExpectedIsAllowed {
				...
				assert.EqualError(
					t,
					err,
					training.ForbiddenToSeeTrainingError{RequestingUserUUID: c.User.UUID(), TrainingOwnerUUID: tr.UserUUID()}.Error(),
				)
			}
		})
	}
