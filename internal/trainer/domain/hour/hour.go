...
func (f FactoryConfig) Validate() error {
	...
	if f.MinUtcHour > f.MaxUtcHour {
		err = multierr.Append(
			err,
			errors.Errorf(
				"MaxUtcHour (%d) can't be before MinUtcHour (%d)",
...
type Factory struct {
	...
	// thanks to that we are always sure that our configuration is not changed in an un-allowed way
	fc FactoryConfig
}
...
func (f Factory) validateTime(hour time.Time) error {
	...
	// AddDate is better than Add for adding days, because not every day has 24h!
